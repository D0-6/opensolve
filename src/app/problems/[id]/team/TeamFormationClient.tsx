    ...
    <div className="space-y-12">
      ...
      <div>
        ...
        {currentMembers.length === 0 ? (
          <div className="bg-white/5 border border-dashed border-white/20 p-8 text-center">
            <p className="text-sm text-zinc-400">You are currently working solo.</p>
            <p className="text-xs text-zinc-400 mt-1">Search below to add teammates, or continue solo.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentMembers.map(m => (
              <div key={m.userId} className="flex items-center justify-between bg-transparent border border-white/10 p-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-zinc-400 font-bold uppercase">
                    {m.name.charAt(0)}
                  </div>
                  <div>
      ...
      {/* Add Teammates */}
      {!isFull && (
        <div className="bg-transparent border border-white/10 p-6 md:p-8">
          <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider mb-4">Invite Teammates</h3>
                  ...
                  <div key={r.userId} className="flex items-center justify-between p-4 bg-white/5 border border-white/10">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-zinc-400 font-mono mt-0.5">{r.githubUrl?.replace("https://github.com/", "") || "No Github"} · {r.country || "Global"}</div>
                      </div>
                    </div>
                    {alreadyAdded ? (
                      <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider px-3 py-1 bg-white/10">Added</span>
                    ) : (
                      <button
