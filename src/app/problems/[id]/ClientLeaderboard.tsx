  ...
  if (loading) {
    return (
      <div className="flex flex-col">
        {[1, 2, 3].map((i) => (
          <div key={i} className="border-b border-white/10 py-6 animate-pulse flex items-center gap-4">
        ...
        {submissions.map((sub, index) => {
          ...
          return (
            <motion.div
              ...
              className={`border-b border-white/10 py-6 flex items-start gap-4 group cursor-pointer transition-colors hover:bg-white/5 ${isExpanded ? 'bg-white/5' : ''}`}
              onClick={() => setExpandedId(isExpanded ? null : sub.rankKey)}
            >
              <div className="w-8 h-8 rounded-full bg-white/10 border border-white/20 flex items-center justify-center font-bold text-zinc-400 shrink-0 mt-1">
                {index + 1}
              </div>
              <div className="flex-1 min-w-0 pr-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5 text-white font-bold bg-white/10 border border-white/10 px-2 py-0.5 rounded-full text-xs ml-4 shrink-0">
                ...
                {isExpanded ? (
                  <div className="mt-4 space-y-4">
                    <div className="prose prose-zinc max-w-none text-sm text-zinc-300 whitespace-pre-wrap bg-transparent p-4 border border-white/10 shadow-sm">
                      {sub.writeup}
                    </div>
                    
                    {sub.demoUrl && (
                      <div className="mt-4">
                        <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest block mb-2">Demo Video</span>
                        ...
                        {sub.demoUrl.includes("youtube.com") || sub.demoUrl.includes("youtu.be") || sub.demoUrl.includes("loom.com") ? (
                          <div className="aspect-video w-full bg-white/10 border border-white/10">
                            <iframe 
                  ...
                  <p className="text-sm text-zinc-400 mt-1 line-clamp-2">{sub.writeup}</p>
                )}
                
                <div className="flex gap-4 mt-3">
                  <a href={sub.githubUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-white transition-colors">
                  ...
                  <div className="ml-auto text-[10px] text-zinc-400 font-bold uppercase tracking-widest group-hover:text-zinc-400 transition-colors">
